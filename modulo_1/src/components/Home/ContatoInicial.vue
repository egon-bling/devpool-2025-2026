...
const { form, carregando, enviado, erros, enviarFormulario } = useContato();
</script>
  ...
  <section id="contato" class="py-24 bg-slate-50 font-sans">
    <div class="container mx-auto px-6 max-w-4xl">
      <div class="text-center mb-12">
        <h2 class="text-3xl md:text-4xl font-display font-black text-slate-900 mb-4">
      ...
      <div class="bg-white p-8 md:p-12 rounded-3xl shadow-xl border border-slate-100 relative overflow-hidden">
        ...
        <Transition name="toast">
          <div v-if="enviado" class="fixed bottom-10 right-10 z-50 w-full max-w-sm">
            <div class="bg-white border border-slate-100 p-5 rounded-2xl shadow-2xl flex items-center gap-4">
        ...
        <form @submit.prevent="enviarFormulario" class="space-y-6" novalidate>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex flex-col">
              ...
              />
              <div v-if="erros.nome" class="mt-2 text-red-600 text-xs flex gap-1">
                 Nome obrigatório.
              </div>
            </div>
            ...
            <div class="flex flex-col">
              ...
              />
              <div v-if="erros.email" class="mt-2 text-red-600 text-xs flex gap-1">
                 E-mail inválido.
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col">
            <label class="text-sm font-bold text-slate-700 mb-2">Telefone</label>
            <input
              v-model="form.phone"
              type="tel"
              placeholder="49991011001"
              :class="['px-4 py-3 rounded-xl border outline-none transition-all', erros.phone ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500']"
            />
            <div v-if="erros.phone" class="mt-2 text-red-600 text-xs flex gap-1">
              Número incompleto (mínimo 10 dígitos).
            </div>
          </div>

          <div class="flex flex-col">
            <label class="text-sm font-bold text-slate-700 mb-2">Nome da Empresa</label>
            <input
              v-model="form.nomeempresa"
              type="text"
              placeholder="Ex: Bingo ERP"
              :class="['px-4 py-3 rounded-xl border outline-none transition-all', erros.nomeempresa ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500']"
            />
          </div>

          <div class="flex flex-col">
            <label class="text-sm font-bold text-slate-700 mb-2">Tamanho da Empresa</label>
            <select
              v-model="form.tamanhoempresa"
              :class="['px-4 py-3 rounded-xl border outline-none transition-all bg-white', erros.tamanhoempresa ? 'border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500']"
            >
